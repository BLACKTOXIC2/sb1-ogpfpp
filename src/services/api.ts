...
export async function generateQuestionsFromImage(imageBase64: string, numQuestions: number): Promise<Question[]> {
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        },
        {
          ...
          content: [{
            type: 'text',
            text: `Generate ${numQuestions} multiple choice questions based on this image. Return ONLY a raw JSON array without any markdown formatting or code blocks. Each question must have:
              - A question string about the content visible in the image
    ...
    if (error instanceof Error) {
      ...
      throw new Error(
        ...
        'Failed to generate quiz questions. Please try again with a different image or number of questions.'
      );
    }
